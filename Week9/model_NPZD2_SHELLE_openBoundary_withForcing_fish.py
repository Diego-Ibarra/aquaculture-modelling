...
def load_defaults():
    ...
    InitCond['n_fish']= 0.020642*10**6 #Initial number of individuals per grid cell (number of fish)
...
def run(days,dt,InitCond,par,forc):
    ...
    for t in range(0,NoSTEPS-1):
        ...
        TotN[t+1] = Phy[t+1] + Zoo[t+1] + SDet[t+1] + LDet[t+1] + NH4[t+1] + \
                    NO3[t+1] + ((B[t+1]*n_muss[t+1])/par['V']) + ((fish[t+1]*n_fish[t+1])/par['V'])
...
def plot(output):
    ...
    ax7.plot(output['time']/365,output['fishTOT'],'c.') 
    ax7.set_ylabel('Nitrogen (mmol N m$^{-3}$)')
    ax7.set_xlabel('Time (years)')
    ax7.set_title('PANEL 7: Aquacultured fish')
    ax7.legend(['FishTOT'])
    plt.show()
    return
