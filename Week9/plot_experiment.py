...
def plot_one_variable(output,var='Phy'):  
    ...
    #ax.set_color_cycle([plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax.set_prop_cycle('color',[plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
...
def plot_ecosystem(output):
    ...
    ax.set_prop_cycle('color',[plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax2.set_ylabel('Zoo \n (mmol N m$^{-3}$)')
#    ax2.set_color_cycle([plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax2.set_prop_cycle('color',[plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax3.set_ylabel('NO3 \n (mmol N m$^{-3}$)')
#    ax3.set_color_cycle([plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax3.set_prop_cycle('color',[plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax4.set_ylabel('NH4 \n (mmol N m$^{-3}$)')
#    ax4.set_color_cycle([plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax4.set_prop_cycle('color',[plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax5.set_ylabel('SDet \n (mmol N m$^{-3}$)') 
#    ax5.set_color_cycle([plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax5.set_prop_cycle('color',[plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax6.set_ylabel('LDet \n (mmol N m$^{-3}$)')
#    ax6.set_color_cycle([plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))]) 
    ax6.set_prop_cycle('color',[plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax7.set_ylabel('Oxy \n (mmol O2 m$^{-3}$)')
#    ax7.set_color_cycle([plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax7.set_prop_cycle('color',[plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax7.set_xlabel('Time (years)')
    ax.set_title('Planktonic Ecosystem')
    
    fig2, (ax9) = plt.subplots(1,1,figsize=(13,5))
    ax9.set_ylabel('B \n (mmol N m$^{-3}$)')
    ax9.set_title('Mussel variables')
    #ax9.set_color_cycle([plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ax9.set_prop_cycle('color',[plt.cm.viridis(i) for i in np.linspace(0, 1, len(output))])
    ...
    for floatLevel in levels:
        ...
        ax9.plot(output[level]['time']/365,output[level]['B'],'-')
    
    ax.legend(legendKeys)
    ax9.legend(legendKeys, loc="upper left")
    plt.show()
    return



def densityVSproduction(multioutput, optimum=0):
    ...
    ax.plot([optimum,optimum], [0,ax.get_ylim()[1]],'g-')
...
def densityVSproductionFISH(multioutput, optimum=0):

    SeedDensity, Production = [], []
    for level in multioutput:
        SeedDensity.append(float(level))
        # Maximum mussel biomass in the last year of simulation
        maxfish = multioutput[level]['fish'][-int(365/0.01):]
        # Initial mussel biomass
        initfish = multioutput[level]['fish'][0]
        # Number of mussels (assuming zero mortality)
        n_fish = multioutput[level]['n_fish'][0]
        # Calculate "Production"
        Production.append((max(maxfish)-initfish)*n_fish)
    ...
    ax.plot(df['SeedDensity'], df['Production'],'ko-')
    ax.plot([optimum,optimum], [0,ax.get_ylim()[1]],'g-')
    ax.set_xlabel('fish density (fish/m$^{3}$)')
    ax.set_ylabel('Max Production (mmol N)')
    ax.set_title('FISH - Density vs Production (max of last year)')
    plt.show()
    return

    
def densityVSoxygen(multioutput, optimum=0):
    import numpy as np
...
def densityVSoxygenFISH(multioutput, optimum=0):
    import numpy as np
    ...
    ax.set_xlabel('Fish density (fish/m$^{3}$)')
    ax.set_ylabel('Oxygen (mmol O2 m$^{-3}$ )')
    ax.set_title('FISH - Density vs Oxygen (median and min of last year)')
    ax.legend(['Median O2','Minimum O2'])
    plt.show()
    return


def densityVSammonia(multioutput, optimum=0):
    import numpy as np
    # NH4
...
def densityVSammoniaFISH(multioutput, optimum=0):
    import numpy as np
    # NH4
    SeedDensity, MedianNH4, MaxNH4, LethalNH4= [], [], [], []
    for level in multioutput:
    ...
    ax.set_ylabel('NH4 (mmol N m$^{-3}$ )')
    ax.legend(['Median NH4','Maximum NH4'])
    ax.set_title('FISH - Density vs Ammonia (median and max of last year)')
    plt.show()
    return
