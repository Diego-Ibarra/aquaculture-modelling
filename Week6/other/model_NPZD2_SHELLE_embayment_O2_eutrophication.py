...
def run(days,dt,InitCond,par):
    ...
    for t in range(0,NoSTEPS-1):
        ...
        dOxydt = dOxydt - ((par['wP']*Phy[t]/par['Z']) * rOxNH4) #Phy remineralization in sediments
        ...
        if Gonad[t+1] < 0:# If negative Gonad, take the biomass from Soma instead <<<<<<< NEW
            Soma[t+1] = Soma[t+1] + Gonad[t+1] # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< NEW
            Gonad[t+1] = 0. # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< NEW         
    ...
    output['Oxy'] = Oxy       # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< NEW
    output['L_Temp'] = L_Temp # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< NEW
    output['L_Salt'] = L_Salt # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< NEW
    output['L_Oxy'] = L_Oxy   # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< NEW
    output['L_Food'] = L_Food # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< NEW 
...
def plot(output):
    ...
    # Plotting # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< ALL THIS SECTION CHANGED
...
if __name__ == '__main__':
    ...
    InitCond['n_muss'] = 4 * 10**8. # number of mussels in basin 
